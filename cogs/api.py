...
class ApiCog(commands.Cog):
    ...
    @commands.command(name = "wikisearch", aliases = ["wiki?"])
    ...
    @commands.command()
    async def wiki(self, ctx, wiki : str):
        id = ctx.author.id
        guildid = ctx.guild.id
        guildid = './guild data/'+ str(guildid) +'.json'
        with open(guildid, 'r') as f:
    ...
    async def reddit(self, ctx, category : str="meme"):
        id = ctx.author.id
        guildid = ctx.guild.id
        ...
        try:
            ...
            await ctx.author.send(f'The Subreddit does not exist! ☹️\nError from : `{guild.name}`')



    @commands.command(name = "meme", aliases = ["lol", "Meme", "MEME"])
    async def meme(self, ctx, category : str="meme"):
        id = ctx.author.id
        guildid = ctx.guild.id
        guild = self.bot.get_guild(guildid)
        guildid = './guild data/'+ str(guildid) +'.json'
